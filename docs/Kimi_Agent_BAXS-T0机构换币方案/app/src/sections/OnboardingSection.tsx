      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="relative">
          ...
          <div className="space-y-8">
            {phases.map((p, i) => (
              <motion.div key={p.phase} initial={{ opacity: 0, x: -30 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ delay: 0.1 + i * 0.12 }}
                ...
                <div className="flex lg:flex-col items-center gap-3 lg:w-[140px] flex-shrink-0">
                  ...
                  <div className="text-center">
                    <div className="text-[10px] font-mono font-bold" style={{ color: p.color }}>{p.phase}</div>
                    <div className="text-xs font-semibold text-[var(--text-primary)]">{p.title}</div>
                  </div>
                </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs font-semibold text-[var(--green)]">Pre-funding（推荐初期）</span>
              </div>
              <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
                先向 Provider 转入 USDT → 获得信用额度 → 再请求付款。资金占用较高但风险最低。
                BAXS 从 Blockchain Wallet 发起 ERC20 转账，Network 监控 12 区块确认后更新额度。
              </p>
            </div>
            <div className="space-y-3">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-[var(--cyan)]" style={{ boxShadow: '0 0 6px var(--cyan)' }} />
                <span className="text-xs font-semibold text-[var(--cyan)]">Post-settlement（成熟期）</span>
              </div>
              <p className="text-xs text-[var(--text-secondary)] leading-relaxed">
                先执行付款 → 后结算 USDT。Provider 授予信用额度，允许临时负债。
                双方互付时可 netting 轧差，理论上可完全抵消。推荐每 8 小时结算一次。
              </p>
            </div>
          </div>
        </motion.div>
