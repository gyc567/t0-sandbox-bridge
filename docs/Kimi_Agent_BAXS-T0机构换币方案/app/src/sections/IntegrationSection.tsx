...
import { Code2, KeyRound, Database, ArrowRightLeft, FileCode, ShieldCheck, Globe2, Wallet, ArrowRight } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5 }} className="text-center mb-16">
          ...
          <p className="mt-4 text-[var(--text-secondary)] max-w-2xl mx-auto">
            BAXS 需要同时实现 OFI 客户端和 Provider 服务端<br />
        ...
        <div className="grid lg:grid-cols-2 gap-6 mb-12">
          {apis.map((api, i) => (
            <motion.div key={api.role} initial={{ opacity: 0, y: 30 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.1 + i * 0.15 }}>
              <div className="glass-panel p-6 h-full">
                <div className="flex items-center gap-3 mb-5">
                  <div className="w-10 h-10 rounded-lg flex items-center justify-center" style={{ background: `${api.color}15` }}>
                    <api.icon className="w-5 h-5" style={{ color: api.color }} />
                  </div>
                  <h3 className="text-sm font-bold" style={{ color: api.color }}>{api.role}</h3>
                </div>
                <div className="space-y-2">
                  {api.endpoints.map(ep => (
                    <div key={ep.method} className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-[rgba(6,10,18,0.5)] border border-[var(--border-color)] hover:border-[var(--gold)]/20 transition-all">
                      <div>
                        <div className="text-xs font-mono font-semibold text-[var(--text-primary)]">{ep.method}</div>
                        <div className="text-[10px] text-[var(--text-muted)]">{ep.desc}</div>
                      </div>
                      <span className="text-[9px] font-mono px-1.5 py-0.5 rounded" style={{ background: `${api.color}15`, color: api.color }}>{ep.type}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ delay: 0.4 }}>
          <h3 className="text-center text-lg font-bold text-[var(--text-primary)] mb-6">核心技术栈</h3>
          <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
            {techStack.map((t, i) => (
              <motion.div key={t.label} initial={{ opacity: 0, scale: 0.9 }} animate={isInView ? { opacity: 1, scale: 1 } : {}} transition={{ delay: 0.5 + i * 0.08 }}
                className="glass-panel p-4 flex items-center gap-3 hover:border-[var(--gold)]/25 transition-all">
                <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" style={{ background: `${t.color}12` }}>
            ...
            {[
              ...
              <div key={s.step} className="relative">
                <div className="bg-[rgba(6,10,18,0.6)] rounded-lg p-3 border border-[var(--border-color)] text-center">
                  ...
                  <div className="text-xs font-semibold text-[var(--text-primary)] mt-1">{s.label}</div>
